...
class SWRegressor(pl.LightningModule):
    ...
    def predict_ts(self, start = None, stop = None, in_data = None, pos = np.array([13.25, 0, 0])):
        ...
        if (pos.ndim == 1): #Were we passed a 1D array corresponding to a static position?
            positions = pd.DataFrame(np.asarray([pos[0] * np.ones(len(timeseries)), pos[1] * np.ones(len(timeseries)), pos[2] * np.ones(len(timeseries))]).T, columns = [pos_keys[0], pos_keys[1], pos_keys[2]])
        elif isinstance(pos, np.ndarray): # is positions an array?
            if len(pos)!=len(timeseries):
                raise RuntimeWarning('Positions array must match length of input timeseries (check with the build_real_input() method).')
            positions = pd.DataFrame(pos, columns = self.pos_norm.keys())
        else: # Is positions a DataFrame? (Anything else would be foolish)
            if len(pos)!=len(timeseries):
                raise RuntimeWarning('Positions array must match length of input timeseries (check with the build_real_input() method).')
            positions = pos.copy()
    ...
    def build_synth_input(
        ...
        for key in self.in_norm.keys():
            if (key == 'BGSE_0')|(key == 'B_xgsm'):
                in_df[key] = bx*np.ones(self.window) #SW BX in nT (GSM coordinates)
            if (key == 'BGSE_1')|(key == 'B_ygsm'):
                in_df[key] = by*np.ones(self.window) #SW BY in nT (GSM coordinates)
            if (key == 'BGSE_2')|(key == 'B_zgsm'):
                in_df[key] = bz*np.ones(self.window) #SW BZ in nT (GSM coordinates)
            if (key == 'V_GSE_0')|(key == 'Vi_xgse'):
                in_df[key] = vx*np.ones(self.window) #SW X velocity in km/s (GSE coordinates)
            if (key == 'V_GSE_1')|(key == 'Vi_ygse'):
                in_df[key] = vy*np.ones(self.window) #SW Y velocity in km/s (GSE coordinates)
            if (key == 'V_GSE_2')|(key == 'Vi_zgse'):
                in_df[key] = vz*np.ones(self.window) #SW Z velocity in km/s (GSE coordinates)
            if (key == 'Np')|(key == 'Ni'):
                in_df[key] = ni*np.ones(self.window) #SW density in cm^-3
            if (key == 'THERMAL_SPD')|(key == 'Vth'):
                in_df[key] = vt*np.ones(self.window) #SW thermal velocity in km/s
            if (key == 'PGSE_0')|(key == 'R_xgse'):
                in_df[key] = rx*np.ones(self.window) #Wind position X GSE (RE)
            if (key == 'PGSE_1')|(key == 'R_ygse'):
                in_df[key] = ry*np.ones(self.window) #Wind position Y GSE (RE)
            if (key == 'PGSE_2')|(key == 'R_zgse'):
                in_df[key] = rz*np.ones(self.window) #Wind position Z GSE (RE)
            if (key == 'SME'):
                in_df[key] = sme*np.ones(self.window) #SuperMAG SMR index (nT)
            if (key == 'SMR'):
                in_df[key] = smr*np.ones(self.window) #SuperMAG SME index (nT)
            if (key == 'tilt'):
                in_df[key] = tilt*np.ones(self.window) #Earth dipole tilt angle (degrees)
        return in_df
    ...
    def build_real_input(self, start, stop):
        ...
        if include_supermag:
            supermag_group = supermag_df.groupby(pd.cut(supermag_df['Epoch'], bins_index)) #Group the SuperMAG data
