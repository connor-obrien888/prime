...
window = 50 #Input window length (100s units)
...
ds = ksc.load_dataset(datapath + 'mms_data.h5', datapath + 'wind_data.h5','sw', in_keys, tar_keys, split_frac=0.2, window=window, stride=stride, inter_frac=int(window*inter_frac), flag=flag, tar_storekey = 'old_targets', conesize = conesize, vx_cut=False, table_cut=True)
...
plt.savefig(plotpath + 'opt_model_loss.pdf', bbox_inches='tight')

#Save the model
model.save(modelpath + 'smokesite/' + name + '.h5')

#Get model outputs for the test dataset
mms_data = pd.read_hdf(datapath + 'mms_data.h5', key = 'old_targets', mode='a') #Load full MMS dataset
mms_data = ksc.nightside_cut(mms_data, conesize=conesize) #Cut to dayside only
list = pd.read_hdf('../data/sw_table_list.h5', key = 'sw_table_list',  mode = 'a')
#mms_data = mms_data.loc[mms_data['Vi_xgse'] < -250] #Cut erroneous slow solar wind
mms_sw = mms_data[(mms_data['region']==2.0)&(mms_data['Epoch'].round('2H').isin(list))] #Select only SW data
mms_test = mms_sw[ds.ds_mask]
mms_test = mms_test.iloc[ds.inds_test]
...
predict_test.to_hdf(datapath + 'cut_outputs.h5', key = 'prime_predict_test')
...
predict.to_hdf(datapath + 'cut_outputs.h5', key = 'prime_predict')
...
predict_full.to_hdf(datapath + 'cut_outputs.h5', key = 'prime_predict_full')
...
predict_bs.to_hdf(datapath + 'cut_outputs.h5', key = 'prime_predict_bs')